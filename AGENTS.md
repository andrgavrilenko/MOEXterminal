...
- [x] Создание ТЗ из технического анализа (Codex)
- [x] Анализ Extension_extract.txt (финансовый анализ от Codex Extension)
...
- [ ] Стадия 4: Деплой на VPS

**Реализовано:**
- 4 основных таба: Арбитраж, Relative Value, Кривые, Акции
- 8 арбитражных суб-табов: Cash-and-Carry, Reverse C&C, Calendar FX, Cross-Instrument, Cross-FX, Cross-Gold, CIP Arb, Funding
- 3 суб-таба Relative Value: Curve Shape, Calendar Commodity, Rate Spread
- Perpetual futures funding analysis (USDRUBF, EURRUBF, CNYRUBF) с минутными свечами
- Авто-обновление (30 сек) + CSV экспорт на всех таблицах
- 23 теста — все проходят

**Не реализовано (в нашей Streamlit-реализации):**
- Условная цветовая раскраска (зелёный/красный фон для ставок)
- Деплой на VPS
- Табы 5–10 из оригинала: Консенсус, Стратегии, Макро, Объёмы, OI Физ/Юр, Кривые (оригинальные)
- Sector Rotation UI (данные есть в snapshot.json, отображение нет)
- Бэктест "Преф/Обычка" (данные есть в backtest_data.json, отображение нет)
- Локальная генерация бэктестов (используется готовый JSON с сервера)
...
| `AGENTS.md` | Этот файл — описание проекта и прогресс |
